param (
    ...
    [string]$vstsPoolName,
	[Parameter(Mandatory = $true)]
    [string]$vstsAgent = "rajaagt111",
    [boolean] $Interactive = $true,
    [string] $Proxy="",
	[Parameter(Mandatory = $true)]
    [string] $vmAdminPassword = "Password@123",
	[Parameter(Mandatory = $true)]
	[string] $vmAdminUserName = "raja",
...
cmd /c .\config.cmd --unattended --url "$vstsAccount" --auth pat --token "$PAT" --pool "$vstsPoolName" --agent "$vstsAgent" --acceptTeeEula --runAsService
